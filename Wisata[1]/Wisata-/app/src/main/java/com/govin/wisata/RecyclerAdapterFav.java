...
public RecyclerAdapterFav(Activity activity, ArrayList<DataRecyclerView> listdata) {
...
//    holder.stok.setText(listdata.get(position).getStok());
    ...
    Glide.with(activity).load(listdata.get(position).getImage()).centerCrop().into(holder.image);

     holder.image.setOnClickListener(new View.OnClickListener() {
         @Override
         public void onClick(View v) {
             Intent i = new Intent(activity, DetailProduk.class);
             ...
             i.putExtra("rating",listdata.get(position).getRating());
             i.putExtra("deskripsi_produk",listdata.get(position).getDeskripsi_produk());
             i.putExtra("image1",listdata.get(position).getImage());
...
//    public RelativeLayout cv;
    ...
    Button lihat;
    TextView nama,harga,ket,radius,stok,hapus;
    MaterialButton ubah;
    ImageView image;
